...
public class Database 
{
    ...
    public static List<String> loadForScoreHistory(String action, String targetName, String fieldName)
    {
        ...
        switch (action)
        {
            case "USER" -> sql = "SELECT DISTINCT NAME FROM user_records ORDER BY ID ASC";
            case "HISTORY" -> sql = "SELECT point.DATE AS DATE, point.SCORE AS SCORE, point.TOTAL_SCORE AS TOTAL_SCORE FROM user_records user JOIN point_records point ON user.ID = point.USER_ID WHERE user.NAME = ? ORDER BY point.ID DESC";
        }
